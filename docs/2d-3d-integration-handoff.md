...
- **Current focus:** Consolidate the completed Tkinter AI generator, native Python importer, React 2D/3D integration, server Vertex bridge, supporting assets/specs/docs, and the production-grade improvement guide on branch `ai-layout-generator` without changing the established serializer/importer/rendering path.
- **AI generator state:** Initial generation remains split by capability: Gemini emits a compact room program, `generate_layout/layout_engine.py` deterministically builds the native-v1 comb shell, Gemini supplies furniture only, autofix handles mechanical geometry, and strict native/design validation is the final fail-closed gate. Refinement still uses bounded full-document rewriting and is the main path targeted by `docs/implementation-guide.md`.
- **Reliability and UX state:** `LayoutSerializer.load_document(..., confirm=True)` remains the only AI apply path and preserves confirmation, backup, rollback, autosave, and existing 3D refresh. Native furniture now resolves `image_path`, `image_name`, or `image_filename`; the existing compass renderer draws a full cardinal rose; and the Tk AI tab streams backend progress into a guarded activity feed.
- **Repository hygiene:** `.gitignore` now excludes nested virtual environments, Python bytecode/cache directories, pytest cache, the local paid `_ai_smoke_tmp.py` harness, and generated source-bundle exports. Those generated files remain available locally but are not tracked. Local `.env` files remain excluded while `.env.example` is included.
- **Latest build fixes:** Removed one premature closing `<div>` in `SandboxView.tsx`, restored the five production type modules from `src/test/types` to `src/types`, and preserved the `'user'` literal type in `Thinker.tsx`. These were shared root-cause fixes for the production build rather than per-error workarounds.
- **Latest validation (offline, no Vertex call):** `layout_engine.py` passed (`11 rooms, 14 doors`), `geometry_autofix.py` passed, `ai_validator.py` passed, and all 70 changed/untracked Python source files compiled in memory. `npm run test:run` passed all 107 tests across 20 files. `npm run build` passed; Vite reports only the existing large-chunk advisory.
- **Next useful action:** Run the Tk editor through `2D_layout/01_run.txt` and complete live visual/provider QA for generate → apply → activity feed → compass rose → refinement. Then implement the independently releasable phases in `docs/implementation-guide.md`, starting with baseline controls and canonical intent rather than replacing the working comb fallback.
- **Remaining boundary:** Live Tk/Vertex QA still requires credentials, billing, and user-visible inspection, so `_ai_smoke_tmp.py` was intentionally not run or committed. Do not create another renderer, bypass `LayoutSerializer`, or route Python-native files through `.hq.json`.