...
const CM_TO_M = 0.01;
// Hide the joint inside the supported beam/deck instead of drawing a wider cap
// that protrudes beyond acute slab corners.
const JOINT_EMBED_M = 0.01;
...
export const PillarMesh: React.FC<{ pillar: Pillar }> = React.memo(({ pillar }) => {
  ...
  const renderedHeight = h + JOINT_EMBED_M;
  const w = pillar.width * CM_TO_M;
  const d = pillar.depth * CM_TO_M;

  return (
    <mesh position={[x, base + renderedHeight / 2, z]} material={material} castShadow receiveShadow>
      {pillar.shape === 'round'
        ? <cylinderGeometry args={[Math.max(w, d) / 2, Math.max(w, d) / 2, renderedHeight, 24]} />
        : <boxGeometry args={[w, renderedHeight, d]} />}
    </mesh>
  );
});
