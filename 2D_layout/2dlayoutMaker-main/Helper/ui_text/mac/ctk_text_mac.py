...
def _import_local_set_window_icon():
    ...
    spec = importlib.util.spec_from_file_location("_mini_autocad_set_window_icon_ui_text_mac", path)
...
class CTKTextTool:
    ...
    def install_global_bindings(self):
        ...
        try:
            ...
            if hasattr(self.app, 'canvas'):
                # Match shape tools: standard right-click (Button-3) + Ctrl+Click, keep Button-2 for compatibility
    ...
    def open_text_dialog(self, initial_text_data=None):
        ...
        # Keep dialog logically attached to the main window so macOS treats it
        # as a child sheet instead of a separate full-screen window.
        try:
        ...
        # On macOS, hide traffic-light minimize/zoom by removing native title bar
        # and provide our own close button in a custom header bar.
        try:
            if platform.system() == "Darwin":
                dialog.overrideredirect(True)
        except Exception:
            pass
        # Keep the dialog visually attached to the main VastuCraft Pro window:
        # center over `self.app.root` when available, otherwise fall back to screen center.
        try:
            dw, dh = 600, 500
        ...
        # Custom header bar with title + close button (since native titlebar is hidden).
        header = ctk.CTkFrame(dialog, fg_color="#f3f4f6")
        header.pack(fill="x", padx=8, pady=(8, 0))
        header.grid_columnconfigure(0, weight=1)
        title_hdr = ctk.CTkLabel(
            header,
            text="Add Text",
            font=("Arial", 14, "bold"),
            text_color="#111111",
        ...
        close_hdr.grid(row=0, column=1, sticky="e", padx=(4, 0))
        ...
        # Title
        title_label = ctk.CTkLabel(form, text="Insert Text", font=("Arial", 16, "bold"))
        title_label.pack(anchor="w", pady=(0, 12))
        
        # Text input (compact)
        text_row = ctk.CTkFrame(form)
        text_row.pack(fill="x", pady=(0, 8))
        ctk.CTkLabel(text_row, text="Text:", font=("Arial", 12)).pack(side="top", anchor="w")
        text_entry = ctk.CTkTextbox(text_row, width=250, height=80)
        ...
        format_label = ctk.CTkLabel(form, text="Formatting:", font=("Arial", 12))
        format_label.pack(anchor="w", pady=(8, 4))

        # Bold and Italic in one row
        format_row = ctk.CTkFrame(form, fg_color="transparent")
        format_row.pack(fill="x", pady=(0, 8))
        bold_cb = ctk.CTkCheckBox(format_row, text="Bold", variable=bold_var, command=update_preview)
        bold_cb.pack(side="left", padx=(0, 15))
        italic_cb = ctk.CTkCheckBox(format_row, text="Italic", variable=italic_var, command=update_preview)
        italic_cb.pack(side="left")

        # Size slider (compact)
        size_label_frame = ctk.CTkFrame(form, fg_color="transparent")
        size_label_frame.pack(fill="x", pady=(8, 4))
        ctk.CTkLabel(size_label_frame, text="Size:", font=("Arial", 12)).pack(side="left")
        size_value_label = ctk.CTkLabel(size_label_frame, text="24px", font=("Arial", 12))
        size_value_label.pack(side="right")
        
        def on_size_change(value):
            size_value_label.configure(text=f"{int(value)}px")
            update_preview()
        
        size_slider = ctk.CTkSlider(form, from_=12, to=72, number_of_steps=60, 
                                    variable=size_var, command=on_size_change)
        size_slider.pack(fill="x", pady=(0, 8))

        # Color picker (compact)
        color_row = ctk.CTkFrame(form, fg_color="transparent")
        color_row.pack(fill="x", pady=(8, 12))
        ctk.CTkLabel(color_row, text="Color:", font=("Arial", 12)).pack(side="left")
        color_entry = ctk.CTkEntry(color_row, textvariable=color_var, width=100)
        ...
        ctk.CTkButton(color_row, text="Pick", width=50, command=pick_color).pack(side="left")
        ...
        ctk.CTkButton(btns, text="Insert", command=on_create, width=100, fg_color="#2563eb", hover_color="#1d4ed8").pack(side="right", padx=(6, 0))
        ctk.CTkButton(btns, text="Cancel", command=dialog.destroy, width=100).pack(side="right", padx=0)
